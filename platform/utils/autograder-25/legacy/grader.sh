...
# get PHY interface to DCN
# the mask is not present as the ip address is used in inter_company_ping()
ZURI_10=$(docker exec -i -w /etc/frr ${ASN}_ZURIrouter bash -c "awk '/PHY-L2.10/,/exit/' frr.conf | awk -F ' *|/' '/ ip address/{print \$4}'")
ZURI_20=$(docker exec -i -w /etc/frr ${ASN}_ZURIrouter bash -c "awk '/PHY-L2.20/,/exit/' frr.conf | awk -F ' *|/' '/ ip address/{print \$4}'")

echo "IP(PHY-L2.10)=$ZURI_10"
echo "IP(PHY-L2.20)=$ZURI_20"
echo -e
...
inter_company_ping () {
    ...
    if [ $LOSS -gt $LOSS_TOL ]; then
        ...
        # check if traceroute only contains 2 hops, and the first hop is a gateway, which equals to PHY's corresponding interface
        if [ $PROTO -eq 4 ]; then
...
# e.g., ROUTER_IFS[1,2]=1 means PHY(ROUTERS[1]) and BASE(ROUTERS[2]) are connected
...
echo "Checking load balancing between BOS and ATL, and between PHY and GENE..."
source venv/bin/activate
SP=$(python sp.py ../config/l3_links.txt ./ospf_arr.txt ./router_arr.txt BOS ATL PHY GENE)
deactivate

# echo "$SP"
LB_CORR=true
SP_MILA_2_MUNI=$(echo "$SP" | awk -F ':' '/SP\(ATL, BOS\)/{print $2}')
SP_MUNI_2_MILA=$(echo "$SP" | awk -F ':' '/SP\(BOS, ATL\)/{print $2}')
SP_ZURI_2_GENE=$(echo "$SP" | awk -F ':' '/SP\(PHY, GENE\)/{print $2}')
SP_GENE_2_ZURI=$(echo "$SP" | awk -F ':' '/SP\(GENE, PHY\)/{print $2}')
# TODO consider more intelligent pattern match, is the networkx output deterministic?
EXP_SP_MILA_2_MUNI=" [['ATL', 'LUGA', 'PHY', 'BOS'], ['ATL', 'LUGA', 'GENE', 'BASE', 'PHY', 'BOS']]"
EXP_SP_MUNI_2_MILA=" [['BOS', 'PHY', 'LUGA', 'ATL'], ['BOS', 'PHY', 'BASE', 'GENE', 'LUGA', 'ATL']]"
EXP_SP_ZURI_2_GENE=" [['PHY', 'BASE', 'GENE']]"
EXP_SP_GENE_2_ZURI=" [['GENE', 'BASE', 'PHY']]"
...
# print 2 random traceroute from ATL to BOS
# and 1 traceroute from PHY to GENE
...
# # get PHY and GENE ipv6 interface to DCN
ZURI_10_6=$(docker exec -i -w /etc/frr 1_ZURIrouter bash -c "awk '/PHY-L2.10/,/exit/' frr.conf | awk -F ' *|/' '/ ipv6 address/{print \$4}'")
ZURI_20_6=$(docker exec -i -w /etc/frr 1_ZURIrouter bash -c "awk '/PHY-L2.20/,/exit/' frr.conf | awk -F ' *|/' '/ ipv6 address/{print \$4}'")
...
echo "IP6(PHY-L2.10)=$ZURI_10_6"
echo "IP6(PHY-L2.20)=$ZURI_20_6"
...
# get tunnel name of PHY and GENE router
declare -A TUN_DEVS
...
print_tunnel_info () {
    ...
    do
        ...
        if [ "$TUN_DEV" != 'sit0' ]; then
            ...
            if ([ "$ROUTER" == "PHY" ] && ([ "$REMOTE_ADDR" != "${ASN}.153.0.1" ] || [ "$LOCAL_ADDR" != "${ASN}.151.0.1" ])) ||
...
if [ "${TUN_DEVS[PHY]}" != "" ]; then
       if [ "$ZURI_TUN6" != "${TUN_DEVS[PHY]}" ]; then
           echo "Tunnel(PHY): incorrect tunnel route"
           TUN_CORR=false
       fi
fi
...
# traceroute from PHY to GENE has been printed in Q1.3, but not checked (only checked shortest paths)
# TODO remove hard-coded PHY and BASE IPs
...
if [ ${#GENE_ZURI_HOPS[@]} -ne 4 ] || [ "${GENE_ZURI_HOPS[1]}" != "${ASN}.0.6.1" ]; then
    # check traceroute between PHY and GENE is still via BASE
    echo "GENE -> PHY: not take desired path."
    ZURI_GENE_LB=false
fi

if [ ${#ZURI_GENE_HOPS[@]} -ne 4 ] || [ "${ZURI_GENE_HOPS[1]}" != "${ASN}.0.1.2" ]; then
    echo "PHY -> GENE: not take desired path."
    ZURI_GENE_LB=false
fi
...
# check output of tcpdump at PHY:port_GENE for DC communication
if [ "$DC_FULL_CONN" == true ]; then
    ...
    echo "Collecting tcpdump result (20 sec) at port PHY:port_GENE:"
